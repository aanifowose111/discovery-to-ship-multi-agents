...
- **Sensitive config** (DigitalOcean Spaces, `.env`, API keys) will be discussed with the user when each app is being built — do not invent placeholders or commit secrets. When checking whether a specific env key is set, use the key-name + emptiness scan (per `guides/product/action-required-methodology.md` §6); never expose values.
