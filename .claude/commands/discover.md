...
- The most recent `status: active` market scan at `market-research/*/scan.md` (look across run folders, newest first) — for territories, anchors, and channel notes
...
5. Write the triage list to `market-research/$RUN_ID/triage.md` (same `$RUN_ID` as the cards folder) with the *Top 3 callout* at the top and `run-id: $RUN_ID` in the frontmatter.
