...
- The most recent `status: active` market scan at `market-research/scan-*.md`
- All `ideas/<slug>.md` with `status: green-lit` or `status: in-validation` (active cards)
...
4. Write the report to `market-research/trends-<YYYY-MM-DD>.md` per §6.

### Stop here — user checkpoint
After writing the report, **stop**. Show the user:

> Trend sweep at `market-research/trends-<YYYY-MM-DD>.md`. {sweep-type}.
>
