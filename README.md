...
| | What | Where |
|---|---|---|
| **Pipeline slash commands** | `/scan`, `/discover`, `/validate-card`, `/scope-mvp`, `/scope-v1`, `/research-design`, `/draft-design-spec`, `/draft-design-brief`, `/start-build`, `/continue-build`, `/ship-app`, `/preview-product`, `/trend-check`, `/reprice`, `/rework`, `/consolidate`, `/infra-cost`, `/revive-card`, `/recollect`, `/build-recap`, `/generate-checklist`, `/read-checklist`, `/check-actions`, `/do-verify`, `/smoke`, `/diff-spec`, `/dev-routes`, `/push-project`, `/deep-debug`, `/caffeinate`, `/stop-caffeinate`, `/team`, `/menu`, `/documentation`, `/setup`, `/status`, `/run-tests`, `/system-check`, `/projects`, `/log`, `/acknowledge-contributing` | `.claude/commands/` |
| **Reviewer assistants** | Product viability / competition / market-segment / pricing / scope reviewers, design-brief / design-spec / design-fidelity reviewers, UI/UX researcher | `.claude/agents/` |
| **Senior-engineer personas** | `senior-software-engineer` (orchestrator), `senior-system-design-engineer`, `senior-database-engineer`, `senior-backend-engineer`, `senior-frontend-engineer`, `senior-desktop-engineer`, `senior-qa-engineer`, `senior-devops-engineer`, `senior-security-engineer`, `senior-debugging-engineer`. Each plays a senior-IC role during the build phase (`senior-debugging-engineer` is the cross-cutting forensics specialist invoked via `/deep-debug`). | `.claude/agents/senior-*.md` |
...
| **Agent-skills skills** (~12 routinely used) | The 12 skills the workspace auto-invokes during the build phase: `api-and-interface-design`, `ci-cd-and-automation`, `code-review-and-quality`, `debugging-and-error-recovery`, `documentation-and-adrs`, `frontend-ui-engineering`, `git-workflow-and-versioning`, `incremental-implementation`, `security-and-hardening`, `shipping-and-launch`, `spec-driven-development`, `test-driven-development`. | Vendored into `.claude/skills/`; re-sync via `scripts/update-agent-skills.sh`. |
| **Helper skills** | `doc-export` (markdown → PDF/DOCX), `web-preview` (render Jinja in Chrome) | `.claude/skills/` |
| **Methodology guides** | Product discovery / validation / MVP scoping; market scan + trend monitoring; funding strategy; Flask scaffold + deploy + storage + auth patterns; React Native scaffold + EAS + store submission; PySide6 desktop scaffold + packaging & distribution; UI/UX research + spec + brief + handoff | `guides/` |
...
| `/scope-v1 <slug>` | Drafts the **v1 brief** (`V1.md`) for a shipped MVP. Captures first-10-users feedback, asks you to pick the design path — `(a) claude-led-continued`, `(b) pro-designer-engaged` (kicks off Phase 3), or `(c) hybrid-light-refresh` — and runs the same two reviewers as `/scope-mvp` against the v1 brief. The chosen design path determines whether the next command is `/research-design` (with `/draft-design-spec` or `/draft-design-brief` after), or `/research-design --light`. |
| `/research-design <slug>` | Runs the UI/UX researcher to produce a design research report (per-surface coverage: public / auth / user / admin / employee dashboards; product-space + platform trends; 3+ visual directions; color/typography options; brand positioning; interactive reference-URL checkpoints with you). Fires for **both** design paths. |
| `/draft-design-spec <slug>` | (Claude-led path) Drafts the implementation-ready `DESIGN_SPEC.md` — typography tokens, exact colors, spacing scale, icon library install, image-asset prompts, responsive breakpoints, per-surface specs. The spec is the build's source of truth (supersedes `frontend-ui-engineering` defaults). |
| `/draft-design-brief <slug>` | (Hired-designer path) Drafts the consolidated design brief (PRD+FRD) for the human designer, with the design-brief-reviewer. |
...
| `/continue-build <slug>` | **Resume** an in-flight build for the named product. The disambiguator for "please continue" when multiple products are in flight. Reads `BUILD_STATUS.md`, finds in-progress (`[>]`) and pending (`[ ]`) subsystems, and invokes the senior-software-engineer in resumption mode (no orientation re-ask). Handles the post-`/rework` `build-status: rework-in-progress` case automatically. |
| `/recollect <slug>` | **Read-only "where are we" synthesis** for the named product. Walks all artifacts (briefs, design research/spec/brief, BUILD_STATUS, team, source tree, audit log) and emits a one-screen report + 2-4 suggested next actions. Use when returning to a product after a break and need to re-orient before deciding what to do. Invokes no subagent; modifies nothing. |
| `/generate-checklist <slug>` | Generate the **fine-grained `CHECKLIST.md`** for a product — the companion to `BUILD_STATUS.md`. Decomposes each MVP/V1 must-have into 3-8 bite-size deliverables with file-path hints (used later for mtime-cached auto-cross-out). Where `BUILD_STATUS.md` is coarse subsystems, `CHECKLIST.md` is fine deliverables. Initial generation; pair with `/read-checklist` for refresh. |
| `/read-checklist <slug>` | Refresh `CHECKLIST.md` — mtime-cached scan that only inspects files modified since the last refresh. Crosses out completed items (matching file-path hints), proposes additions for newly discovered work (capped at 5 per pass; user confirms each), updates the Scope changes log. **Auto-triggered by the build orchestrator** after every `BUILD_STATUS.md` subsystem flips to `[x]`. |
| `/check-actions <slug>` | Refresh `ACTION_REQUIRED.md` for a product — the per-product tracker for external/third-party items only the user can obtain (API keys, OAuth apps, image-asset URLs, domain DNS). Distinct from `CHECKLIST.md` (Claude's deliverables) — `ACTION_REQUIRED.md` is the only file for user-only work. Runs a key-name + emptiness scan of `<product-folder>/.env` (no values exposed to context), auto-cross-outs items whose env keys are now set, reports newly completed + still pending (split blocking vs non-blocking). Auto-created by `/draft-design-spec` Step 5 alongside CHECKLIST.md; auto-appended-to during build when specialists surface new dependencies; auto-refreshed by the orchestrator after every subsystem flip. |
| `/push-project <slug> [--init\|--status\|--remote\|-m "<msg>"]` | Push a product (`web-apps/<slug>/`, `mobile-apps/<slug>/`, `desktop-apps/<slug>/`) to its own **independent GitHub repository**, separate from the parent workspace repo. The parent's `.gitignore` ignores the nested folders, so each can have its own `.git/`, remote, history, branches. `--init` for first-time setup (stack-specific `.gitignore`, `README.md`, optional `LICENSE`, sets remote, initial commit + push); `--status` for read-only check; no flags for routine push. **Mandatory safety scans:** refuses staged `.env` files; scans for AWS keys / GitHub tokens / Stripe keys / private keys; refuses plain `--force` to `main`. Per-commit Co-Authored-By trailer ask. |
| `/deep-debug <slug> [focus-area]` | Invoke the **`senior-debugging-engineer`** for deep root-cause analysis on a cross-cutting failure (flaky test, backend+frontend race, deploy failure with non-obvious cause, intermittent production bug). Runs a hypothesis-driven 5-phase process (Reproduce → Hypothesize 2-4 candidates → Test each → Identify root cause → Recommend fix) and returns a structured report. **Does NOT write the fix** — the relevant `senior-X-engineer` does that, against the report. Optional focus narrows the investigation. |
| `/caffeinate` and `/stop-caffeinate` | **macOS-only** display + system sleep prevention. `/caffeinate` runs `caffeinate -d -i` in the background (PID tracked at `~/.claude/caffeinate.pid`); `/stop-caffeinate` reads the PID, sends SIGTERM, removes the pidfile. Idempotent. Pair with the break-reminder hook (`.claude/hooks/break-reminder.sh`) — wired to `UserPromptSubmit`, fires after 2h sessions (1h throttle) → Claude surfaces a check-in via `AskUserQuestion` at the next natural conversation boundary. |
...
| `/projects` | List every discovery-cycle project in your workspace (keyed by run-id) with a one-line summary of each, then offer actions on a chosen one — primarily **delete** (which walks `ideas/`, `ideas/killed/`, `market-research/`, `web-apps/`, `mobile-apps/`, `desktop-apps/`, and `generated/` for matching artifacts). Deletion is **irreversible** (files bypass the Trash) and gated by a two-step user confirmation flow. Wraps `scripts/projects.py`. Use to clean up abandoned discovery cycles, verification tests, or any project you no longer need. |
...
| `/rework <slug> <changes>` | Rework an idea card (and optionally its downstream MVP brief / V1 brief) based on user-described changes. **Originals are NEVER overwritten until the user commits.** Proposed changes land in temp files (`<slug>-temp.md`, `MVP-temp.md`, `V1-temp.md`); reviewers run against the temps; user inspects verdicts; commit replaces originals with temps. REJECT verdicts surface a suggested approach to get passing; user can also override a REJECT, in which case a consequences explainer + one-sentence justification is required and recorded in the `rework-applied` audit-log entry. Use when an idea/scope/MVP needs substantive changes after initial reviewers ran. |
| `/consolidate <slug>` | Check alignment across an idea card, its validation report, its scoping report, its MVP brief, and (if present) its V1 brief — surface every misalignment in a numbered table with suggested resolutions, ask the user for permission (per row or all-at-once) before consolidating, then re-run the relevant reviewers against the consolidated artifacts. Audit-logged as `consolidation-applied`. Use when artifacts have drifted (e.g., MVP must-have doesn't trace to the card, validation chosen-price differs from MVP `priced-at:`). Typical sequence: `/consolidate` first to clean drift, then `/rework` if substantive changes are needed. |
| `/infra-cost <slug> [--save \| --users=A,B,C \| --include-v1]` | Estimate the infrastructure cost of running an MVP (or V1) — minimum / medium / max scenarios by user base. Reads MVP.md (and V1.md if `--include-v1`), extracts stack and infrastructure decisions, fetches current vendor pricing live, and assembles a cost table per cost item with flags for **recurring vs. one-time** and **user-dependent vs. fixed**. Default tiers `10, 100, 1000` (override with `--users=A,B,C`). Output also breaks down "fixed monthly burn" (admin cost the maintainer pays regardless of users) vs. "variable burn" (user-dependent). Surfaces caveats + assumptions. Optionally writes the report to `<stack>-apps/<slug>/INFRA_COST.md` with `--save`. |
...
- **Post-MVP polish via `/scope-v1 <slug>`.** After your MVP ships and the riskiest assumption holds, `/scope-v1` is the gate to the polished v1. It captures first-10-users feedback, lets you pick the design path — (a) claude-led continued, (b) engage a professional UI/UX designer (kicks off the full designer-engagement phase), or (c) hybrid light refresh — and drafts `V1.md` next to `MVP.md` with the same two-reviewer pass as `/scope-mvp`.
...
| `lint_pipeline.py` | Validate pipeline state consistency (frontmatter, status alignment, `@path` cross-references, required sections in validation reports — handles both per-reviewer §5 and integrated §7 shapes — category-scoped slug-collision rule that recognizes the expected post-`/scope-mvp` coexistence). |
| `check_slug.py` | Verify a product slug is available across `ideas/`, `ideas/killed/`, `web-apps/`, `mobile-apps/`, `desktop-apps/`. Used by `/discover` and `/scope-mvp` before writing new artifacts. Also imported by `new_idea_card.py` and used directly when claiming a slug. |
...
| `audit_log.py` | Read / write the personal-space audit log at `user-context/audit-log.jsonl` (gitignored). Subcommands: `add`, `list`, `delete`, `clear`, `has`. Supports types `onboarding-skip`, `project-delete`, `card-kill`, `card-revive`, `build-milestone`, `user-note`. **Backs the `/log` slash command.** The `has` subcommand gates the Rule A onboarding re-prompt in CLAUDE.md. |
| `team.py` | Manage per-product senior-engineer team-member names at `<web-apps\|mobile-apps\|desktop-apps>/<slug>/team.json` (gitignored). Subcommands: `get`, `set`, `list`, `init`, `reset`, `roles`, `path`. Names validated 1-30 chars (letters / digits / spaces / hyphens / apostrophes). The 9 roles are fixed; no `delete` subcommand because the roles are workflow-critical. **Backs the `/team` slash command.** Used by `/start-build` for just-in-time naming prompts. |
| `projects.py` | Manage discovery-cycle projects: `list` (all projects with summaries), `show <run-id>` (dry-run of what would be deleted), `delete <run-id> --force` (actually delete). Identifies a project by run-id and walks every artifact keyed to it (`ideas/`, `ideas/killed/`, `market-research/`, `web-apps/<slug>/`, `mobile-apps/<slug>/`, `desktop-apps/<slug>/`, `generated/**/<slug>`). **Destructive when `--force` is passed.** Backs the `/projects` slash command (which adds two interactive confirmations before invoking `--force`). |
