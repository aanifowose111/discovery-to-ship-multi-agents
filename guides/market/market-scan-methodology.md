...
| **Adjacency signals** | What workflows surround the user's already-shipped territory? Pull from `user-context/INTERESTS.md` for the *current* user's strengths; each shipped project has adjacent workflows worth surfacing. (For the maintainer: findvil for job-search adjacency, fijara for multi-tenant-SaaS service-business adjacency.) |
